...
COMMENT ON COLUMN "MEMBER"."MEMBER_TEL" IS '휴대폰 번호(-없이)';
...
COMMENT ON COLUMN "MEMBER"."MEMBER_DEL_FL" IS '탈퇴여부(N: 탈퇴X, Y탈퇴 O)';
...
COMMENT ON COLUMN "BOARD"."BOARD_NO" IS '게시판 번호(SEQ_BOARD_NO';
...
COMMENT ON COLUMN "BOARD"."B_UPDATE_DATE" IS '게시글 수정일(수정시 UPDATE)';
...
COMMENT ON COLUMN "BOARD_LIKE"."BOARD_NO" IS '게시판 번호';
...
COMMENT ON COLUMN "BOARD_TYPE"."BOARD_CODE" IS '게시판 종류별 코드번호(SEQ_BOARD__CODE)';
...
-- 회원 샘플데이터 삽입
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'user01@kh.or.kr','pass01!','유저일','01012345678','04540,, 서울시 중구 남대문로 120,, 2층',DEFAULT,DEFAULT,DEFAULT,DEFAULT);

INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'user02@kh.or.kr','pass02!','유저이','01022222222','04540,, 서울시 중구 남대문로 120,, 3층',DEFAULT,DEFAULT,DEFAULT,DEFAULT);
...
-- 로그인 SQL
SELECT MEMBER_NO, MEMBER_EMAIL,MEMBER_NICKNAME,MEMBER_TEL,MEMBER_ADDRESS,PROFILE_IMG, AUTHORITY,TO_CHAR(ENROLL_DATE,'YYYY"년" MM"월" DD"일" HH24"시)" MI"분" SS"초"') AS ENROLL_DATE
FROM "MEMBER"
...
UPDATE "MEMBER" SET
MEMBER_PW  = '$2a$10$2aTzULMDDmypjjD8uVCekO/5exi2atqbNCdt3kJd0m1UHuVlshuj.';

COMMIT;

-- bycypt 암호화 적용시 로그인 SQL
SELECT MEMBER_NO, MEMBER_EMAIL,MEMBER_PW,MEMBER_NICKNAME,MEMBER_TEL,MEMBER_ADDRESS,PROFILE_IMG, AUTHORITY,TO_CHAR(ENROLL_DATE,'YYYY"년" MM"월" DD"일" HH24"시)" MI"분" SS"초"') AS ENROLL_DATE
FROM "MEMBER"
WHERE MEMBER_DEL_FL = 'N'
AND MEMBER_EMAIL='user01@kh.or.kr'
