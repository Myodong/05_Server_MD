...
public class StudentDAO {
	...
	public List<Student> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				String deoartmentName = rs.getString("DEPARTMENT_NAME");
				
				stdList.add(new Student(studentNo, studentName, studentAddress, deoartmentName));
			}
	 ...
	 * @throws Exception
	 */
	public List<Student> selectDepartmentServlet(Connection conn, String inputDept)throws Exception {
		
		List<Student> stdList = new ArrayList<>();
		
		try {
			String sql = prop.getProperty("selectDepartmentServlet");
